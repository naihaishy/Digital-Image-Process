...
void CImageProcessView::BilinearInterpolation(int Width, int Height)
{
	...
	int DstLineByte = (Width*m_nBitCount + 31) / 32 * 4; //目标位图每行大小
...
3.连续化旋转将导致空白区域不断扩大 图片不断变大 2017.10.7  ??? 在对图像进行旋转前 进行裁剪
...
void CImageProcessView::OnFileOpen()
{
	...
	//CString filter;
	//filter = _T("所有文件(*.bmp,*.jpg,*.gif,*tiff)|*.bmp;*.jpg;*.gif;*.tiff| BMP(*.bmp)|*.bmp| JPG(*.jpg)|*.jpg| GIF(*.gif)|*.gif| TIFF(*.tiff)|*.tiff||");
...
void CImageProcessView::OnFileSave()
{
	...
	/*CString filter;
	filter = _T("所有文件(*.bmp,*.jpg)|*.bmp;*.jpg| BMP(*.bmp)|*.bmp| JPG(*.jpg)|*.jpg||");*/
	...
	if (dlg.DoModal() == IDOK) {
		...
		if (nFilterIndex == 1)            //当用户选择文件过滤器为".BMP"时
		{
			...
			SaveBmp(lpstr);                  //保存bmp图片 就是一个写出图片的过程 
